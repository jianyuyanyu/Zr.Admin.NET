...
namespace ZR.ServiceCore.Services
{
    public interface ISysOperLogService
    {
        ...
        /// 与 <see cref="GetOperHealthMetrics"/> 同口径：operName 非空时只统计该用户。
        /// </summary>
        /// <param name="input">时间范围参数</param>
        /// <param name="dimension">维度 Key，取值见 OperDimensionKinds，非法值降级为 module</param>
        /// <param name="operName">限定操作人，非空时只统计该用户（非管理员口径）</param>
        /// <param name="topN">返回条数上限，超出部分合并为"其他"（风险等级维度固定 3 档不合并）</param>
        /// <returns>该维度的聚合项</returns>
        List<OperDimensionStat> GetOperDimensionStats(LogAiAnalysisInput input, string dimension, string operName = null, int topN = 12);
    }
}
